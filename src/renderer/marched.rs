...
impl Renderer for MarchedRenderer {
    /// Recreates the marched renderer's framebuffers, pipeline, and swapchain, all of which depend
    /// on the window size.
...
/// Gets the render pass to use with the marched renderer. In Vulkan, a render pass is the set of
...
/// Gets the graphics pipeline containing the marched vertex and fragment shaders.
pub fn get_pipeline(
    render_pass: &Arc<RenderPass>,
    dimensions: [u32; 2],
    device: &Arc<Device>,
