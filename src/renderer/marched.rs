...
use crate::shaders::{albedo_vert, ambient_frag};
use crate::shaders::{expand_vec3, marched_frag, ShaderModulePair};
...
pub struct MarchedRenderer {
    ...
    point_light_buf_pool: CpuBufferPool<marched_frag::UPointLightData>,
    point_light_buf: Arc<CpuBufferPoolSubbuffer<marched_frag::UPointLightData>>,
    metaball_buf_pool: CpuBufferPool<marched_frag::UMetaballData>,
    metaball_buf: Arc<CpuBufferPoolSubbuffer<marched_frag::UMetaballData>>,
...
impl MarchedRenderer {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>) -> Self {
        ...
        let point_light_buf_pool = CpuBufferPool::<marched_frag::UPointLightData>::uniform_buffer(
            buffer_allocator.clone(),
        );
        let metaball_buf_pool =
            CpuBufferPool::<marched_frag::UMetaballData>::uniform_buffer(buffer_allocator.clone());
        let vp_buf_pool =
            CpuBufferPool::<albedo_vert::UCamData>::uniform_buffer(buffer_allocator.clone());

        let point_light_buf =
            point_light_buf_pool
                .from_data(marched_frag::UPointLightData {
                    data: unsafe {
                        to_partially_init_arr::<
                            MAX_POINT_LIGHTS,
                            Padded<marched_frag::UPointLight, 12>,
                        >(&Vec::new())
                    },
                    len: 0,
                })
        ...
        let metaball_buf = metaball_buf_pool
            .from_data(marched_frag::UMetaballData {
                data: unsafe {
        ...
        let framebuffers =
            window_size_dependent_setup(&base.images, render_pass.clone(), &mut base.viewport);
        ...
        let dummy_vertex_buf = Buffer::from_iter(&buffer_allocator);
        ...
        let lighting_set = PersistentDescriptorSet::new(
            ...
            [
                ...
                WriteDescriptorSet::buffer(
                    1,
                    self.ambient_light_buf
                        .as_ref()
                        .expect("No ambient light added")
                        .clone(),
                ),
            ],
        )
    ...
    pub fn set_objects(&mut self, objects: &Vec<Metaball>) {
        let objects = objects
            .iter()
            .map(|obj| {
                Padded::from(marched_frag::UMetaball {
    ...
    pub fn set_point_lights(&mut self, point_lights: &Vec<PointLight>) {
        let point_lights = point_lights
            .iter()
            .map(|light| {
                Padded::from(marched_frag::UPointLight {
...
unsafe fn to_partially_init_arr<const MAX_LEN: usize, T: Copy>(values: &Vec<T>) -> [T; MAX_LEN] {
    ...
    if values.len() > MAX_LEN {
        panic!(
            "Only {} point lights may be added to the scene at one time",
            MAX_LEN
        )
    }
